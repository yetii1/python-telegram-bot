...
async def run_web_server(bot, port=8080):
    ...
    await site.start()
    logger.info(f'Web API server running on port {port}')
    
    # Бесконечный цикл чтобы сервер не закрывался
    await asyncio.Event().wait()

async def main_async(application: Application) -> None:
    """Запускает бот и веб-сервер одновременно."""
    port = int(os.environ.get('PORT', 8080))
    
    # Запускаем веб-сервер в отдельной задаче
    web_task = asyncio.create_task(run_web_server(application.bot, port))
