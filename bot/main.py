"""Telegram bot with embedded API server."""
import logging
import threading
import asyncio
...
def api_thread(bot):
    ...
    loop.run_until_complete(run_api_server(bot))
...
def main() -> None:
    ...
    thread = threading.Thread(target=api_thread, args=(application.bot,), daemon=True)
    thread.start()
